...
export default function ServicePage() {
  ...
  // Fetch user's CVs and cover letter templates
  useEffect(() => {
    const fetchDocuments = async () => {
      if (serviceId === 'jobs' && user?.email && user?.id) {
        ...
        try {
          ...
          // Fetch cover letter templates from database
          const templatesResponse = await fetch(`/api/cover-letter-templates?userId=${user.id}&t=${Date.now()}`);
          let dbTemplates = [];
          if (templatesResponse.ok) {
            const templatesResult = await templatesResponse.json();
            console.log('📄 Cover letter templates fetched:', templatesResult.templates);
            if (templatesResult.templates) {
              // Get active templates from database
              dbTemplates = templatesResult.templates
            ...
            console.error('❌ Template fetch failed:', templatesResponse.status);
          }

          // Also fetch existing PDFs directly from Supabase Storage
          try {
            ...
            console.log('📄 Storage files result:', { files: storageFiles, error: storageError });

            let storagePdfs = [];
            if (!storageError && storageFiles) {
              // Get file paths that are NOT already in database templates
              const dbFilePaths = new Set(dbTemplates.filter(t => t.type === 'pdf').map(t => t.file_path));
              
              storagePdfs = storageFiles
                .filter(f => f.name !== '.emptyFolderPlaceholder' && f.name.toLowerCase().endsWith('.pdf'))
                .filter(f => !dbFilePaths.has(`${user.id}/${f.name}`))
                .map(f => ({
                  id: `storage_${f.name}`, // Prefix with storage_ to avoid ID conflicts
                  name: f.name.replace(/\.[^/.]+$/, ''), // Remove extension for display
                  type: 'pdf',
                  file_path: `${user.id}/${f.name}`,
                  file_name: f.name,
                  isFromStorage: true // Mark as storage file
                }));
              
              console.log('📄 Storage PDFs found:', storagePdfs.length);
            }

            // Combine database templates and storage PDFs
            const allCoverLetters = [...dbTemplates, ...storagePdfs];
            console.log('📄 Total cover letters:', allCoverLetters.length, '(DB:', dbTemplates.length, ', Storage:', storagePdfs.length, ')');
            setCoverLetters(allCoverLetters);
          } catch (error) {
            console.error('Error fetching storage files:', error);
            // If storage fetch fails, just use database templates
            setCoverLetters(dbTemplates);
          }
        } catch (error) {
                    ...
                    >
                      ...
                      {coverLetters.map((template) => (
                        <option key={template.id} value={template.id}>
                          {template.name} ({template.type === 'pdf' ? 'PDF' : 'Text Template'})
                        </option>
                      ))}
                    </select>
                    {coverLetters.length === 0 && (
                      <p className="text-xs text-gray-500 mt-1">No cover letter templates created yet. Go to Customer Dashboard to create templates.</p>
                    )}
                  </div>
                </div>
